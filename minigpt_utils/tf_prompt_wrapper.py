...
class Prompt:
    ...
    def update_context_embs(self):
        if len(self.text_embs) == len(self.img_embs):
            self.context_embs = self.generate_context_embedding(self.text_embs, self.img_embs)
        else:
            self.context_embs = []
    ...
    def generate_text_embedding(self, text_prompts):
        ...
        for item in text_prompts:
            prompt_segs = item.split('<ImageHere>')
            seg_tokens = [
                self.model.llama_tokenizer(
                    seg, return_tensors="pt", add_special_tokens=i == 0).to(self.device).input_ids
                for i, seg in enumerate(prompt_segs)
            ]
            embs = [self.model.llama_model.model.embed_tokens(seg_t) for seg_t in seg_tokens]
        ...
        for items in img_prompts:
            ...
            for img in items:
                if isinstance(img, tf.Tensor):
                    img_torch = torch.from_numpy(img.numpy()).to(self.device)
                else:
                    img_torch = img
                feats, _ = self.model.encode_img(img_torch)
                embs.append(feats)
            img_embs.append(embs)
        return img_embs

    def generate_context_embedding(self, batch_text_embs, batch_img_embs):
        assert len(batch_text_embs) == len(batch_img_embs), "Unmatched batch size of text and image prompts"
        ...
        for i in range(batch_size):
            ...
            if num_text_segs == 0 and num_img_segs == 0:
                mixed_embs_list = [torch.zeros([1, 0, 0])]
            elif num_text_segs == 0:
                mixed_embs_list = img_embs
            elif num_img_segs == 0:
                mixed_embs_list = text_embs
            else:
                s = t = 0
                mixed_embs_list = []
                while s < num_text_segs and t < num_img_segs:
                    mixed_embs_list.append(text_embs[s])
                    mixed_embs_list.append(img_embs[t])
                    s, t = s + 1, t + 1
                if s < num_text_segs:
                    mixed_embs_list += text_embs[s:]
                if t < num_img_segs:
                    mixed_embs_list += img_embs[t:]
            mixed_embs = torch.cat(mixed_embs_list, dim=1)
