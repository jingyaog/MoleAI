...
class JailbreakDetector(nn.Module):
    def __init__(self, base_model_name="openai/clip-vit-base-patch32"):
        ...
        # We use a simple MLP. Input is 768 (CLIP embedding size).
    ...
    def forward(self, pixel_values):
        # 1. Pass image through frozen CLIP
        outputs = self.encoder(pixel_values=pixel_values)
        
        # 2. Get the pooled feature vector (The "Gist" of the image)
...
class PairedDataset(Dataset):
    def __init__(self, clean_dir, adv_dir, processor):
...
def main():
    ...
    # Load Processor (Transformations)
    # This must match the model name in the class above
    processor = CLIPImageProcessor.from_pretrained("openai/clip-vit-base-patch32")

    # Load Data
    full_dataset = PairedDataset(args.clean_dir, args.adv_dir, processor)
